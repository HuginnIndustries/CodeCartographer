...
| **Split `core/broadside.ts`** — 4,300 lines into lenses / repo / client / models / state / submit / collect / post-passes / render | [#339](https://github.com/HuginnIndustries/CodeCartographer/issues/339) | **Shipped** (0.26.0): `core/broadside.ts` is a barrel over fourteen modules in `core/broadside/`; exports unchanged, suite untouched. The 2026-09-15 self-audit's #371 removed the last two structural edges and `tests/module-graph.test.mjs` pins the measured layer order and that the graph is acyclic (#391) |
