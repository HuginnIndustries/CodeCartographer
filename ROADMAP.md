...
| **Verdicts feed the work order** — triage and synthesis read `verified.json` when it exists | [#338](https://github.com/HuginnIndustries/CodeCartographer/issues/338) | Open. Confirmed first, discarded dropped, `not-a-defect` in its own section; needs an explicit regenerate since post-passes are claimed per run |
| **Split `core/broadside.ts`** — 4,300 lines into lenses / repo / client / models / state / submit / collect / post-passes / render | [#339](https://github.com/HuginnIndustries/CodeCartographer/issues/339) | Open. Pure refactor behind the `core/index.ts` barrel; one PR per module or one move-only PR; the test suite is the net |
