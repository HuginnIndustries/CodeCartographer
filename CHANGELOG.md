...
- **Pi: a phase is reserved before its prompt is built, so two runs cannot spawn it twice.** `/codecarto-next` checked whether the phase was running and then called into a prelude — the prompt build and, with `--llm-steer`, a model round-trip — before registering the run, so a second `/codecarto-next` (or a manual run racing `--auto`) in that window found the phase unregistered and spawned a second sub-agent for it; both wrote the same output and both auto-completed. The check and the reservation are now one synchronous step at the top of `runSinglePhase`; a duplicate call gets `Phase X is already running.` as an error result, and a prelude that fails releases the reservation. Found by the 2026-09-15 self-audit (#359).

- **Every writer of the library index takes the publish lock, and a list never writes it.** `codecarto_library_reindex` rebuilt `index.yaml` and `INDEX.md` with no lock while publishes serialize on `.publish.lock`, so a reindex that scanned the entry directories before a publish's rename and wrote after the publish's own reindex dropped the new version from the index until the next rebuild; and `codecarto_library_list` rewrote a corrupt index unlocked — a read command that writes. `reindex` now takes the publish lock itself (the publisher, which already holds it, says so), and a list whose `index.yaml` is missing or does not parse answers from the entry directories without writing, reports `indexState` (`indexed`, `missing`, `unparseable`) in its structured content, and tells the caller to run a reindex. Found by the 2026-09-15 self-audit (#357).

- **Scaffold refresh runs under the status lock, writes each file atomically, and logs each version transition once.** `refreshScaffold` overwrote the pipelines, GUIDE, and VALIDATE with `copyFile` and no lock, so a completion in flight — which re-reads the active pipeline inside the lock — could see a half-written pipeline or a replaced one; a template file that could not be read failed the refresh partway with no record; and every run appended a THREAD_LOG line. It now takes the status lock for the whole refresh, reads every packaged file before writing any (a template that cannot be read leaves the scaffold as it was), writes each through `atomicWriteFile`, and appends the THREAD_LOG line once per version transition — a same-version restore is logged once, a repeat adds nothing, and both surfaces say which happened (`threadLogEntryAppended`). Found by the 2026-09-15 self-audit (#356).

- **The framework lock is a queue of owned tickets; a live holder is never broken and a dead one never blocks.** `acquireLock` used one shared file: waiters broke a lock older than 60 s by removing that path and re-creating it, which is a race between any two waiters that saw it stale (#344 closed one level of it and left the next), and a fixed age broke *live* holders — a publish across a full `reindex` lost its lock at 60 s and reopened the version race #240 fixed. Each waiter now writes its own ticket beside the lock (`<lock>.t.<order>-<pid>-<token>`), refreshes it on a timer while it waits and while it holds, and the holder is the first ticket whose owner is alive and refreshing; a ticket whose owner is dead is removed at once, one whose owner stopped refreshing for `STALE_LOCK_MS` after that — each by its own unique name, so nothing shared is ever removed and re-created. Ordering is Lamport's bakery (a waiter announces it is choosing before it writes its ticket), so no waiter can be overtaken between taking a number and writing it. A plain lock file left by an older process is honoured while fresh and removed when stale. `acquireLock` gains `{ timeoutMs, staleMs }`; `BREAK_LOCK_STALE_MS` and the removal lock are gone. Found by the 2026-09-15 self-audit (#355). A ticket's number is one more than the largest ticket its owner can see when it chooses, not the wall clock: two arrivals in the same millisecond could otherwise sort the later one ahead of the holder.
...
- **The defect lens is asked for the trigger up front.** The rubric the `verify` pass applies — a finding is a reachable failure; name the concrete input, call site, or sequence that reaches it and what then goes wrong; a cast, assertion, `any`, or non-null `!` every visible caller satisfies, a hypothetical about a runtime the project does not target, or a style observation is not a defect and, if worth a note, goes at severity low under the pattern name `type-hygiene` — is now in the scan's own prompt, after the per-language pattern list. Measured on this repository against the previous prompt, two runs per arm on `google/gemini-3.7-flash:batch`, findings judged by hand against the source: the scan returns half as many findings (24 and 26 → 10 and 13), medium-severity precision roughly doubles (2/12 and 3/13 → 1/4 and 3/6), and the number of real defects a run turns up is the same on average (2, 3 → 1, 4) — either prompt misses a real one some runs, which is what `verify` and a second run are for. Same cost per run. The reading guide says to read `type-hygiene` rows as notes.
...
- **`/codecarto-library-init` refuses an incomplete or invalid `--namespace`.** `--namespace` as the last token read as "no namespace": the library was initialized unnamespaced and the config written without one, with no word about the flag — one of the two findings the 2026-09-13 `verify` pass confirmed on this repository. The flag is now either complete or refused (`--namespace needs a name`), an unknown flag or a second path is refused with the usage line, and the value is checked against the rule publish applies to it later (lowercase ASCII, starts with a letter, at most 64 characters) before it is written into the user config. `codecarto_library_init` applies the same rule.
- **Amendments judge pipeline completeness on the state read under the lock.** `applyAmendment` checked `resolvePipelineOutcome` on a read taken before `updateStatusAtomically` took the status lock, so a status change that landed in between — a pipeline switch, a re-init, a completion rolled back — was amended over as if the pipeline were still complete; the other finding the 2026-09-13 `verify` pass confirmed, and the sweep's top-ranked one. The refusal now runs on the locked read, with the same messages; a refused amendment writes nothing.
- **Two waiters breaking the same stale lock can no longer both take it.** `acquireLock` broke a lock older than `STALE_LOCK_MS` with an unconditional `rm` after a `stat`; two waiters that both saw the stale file — the normal case, since every waiter polls on the same cadence — both removed it, and the second `rm` landed after the first waiter had re-created the file, so both held the lock at once (#342; found by a Broad-Side defect run on this repository and confirmed by `verify`). Every removal now happens under a removal lock (`<lock>.break`) and re-checks what it removes there: a stale break confirms the age under it, a release confirms its token under it, and a file can only be created while the path is free, so what a remover verified is what it removes. A removal lock left by a crashed process is cleared after `BREAK_LOCK_STALE_MS`; a release that cannot take the removal lock in time falls back to the token-checked removal alone — the guarantee before this change, never less.
- **Pi: re-running a phase within 30 s of its last run keeps its live widget entry.** The runner clears a phase's activity 30 s after it ends so `/codecarto-status` can still show it ran; that timer deleted whatever the map held for the phase, so a re-run started inside the window — `/codecarto-next` right after an abort or an error — lost its running entry: the widget dropped it, and the re-entry guard would have let a second sub-agent start on the same phase (#343; found by a Broad-Side defect run on this repository and confirmed by `verify`). The linger clear is now tied to the run it was scheduled for and does nothing when a newer run holds the entry.
...
- **Two Broad-Side operations running at once silently erased each other's runs.** `state.json` was written by overwriting the whole file with the in-memory snapshot the operation had loaded when it started. A collect holds that snapshot for as long as it polls — the better part of an hour on a real batch — so every run recorded by any other operation in the meantime was wiped on the next checkpoint, and every checkpoint after it wrote the loss back again. The erased run's paid results stayed on disk as files while the run itself became unreachable: `list` could not see it, and `collect` finds its run by position in `state.runs`, so there was no way to resume or bill it. Caught on a live pair of overlapping scans of this repository. A submit at 23:35 recorded its run; a collect that had loaded state before it wrote back at 00:08 and the run vanished, its eleven completed lens results orphaned. Six minutes later the second collect wrote *its* stale snapshot, restoring the lost run and rolling a finished run's recorded cost backwards from `$0.422379` to `$0.372623` — a completed run's spend record silently rewritten by an unrelated operation. Checkpoints now merge one run into whatever is on disk at that moment, under the same lock discipline `status.yaml` has always used, and write through a temp file and rename so an interrupted write cannot truncate the state file. Merging by run id also self-heals: a run erased by an older writer is restored by its own next checkpoint. `saveBroadsideState` keeps its overwrite semantics for seeding a fresh workspace, and now says so.

- **A post-pass abandoned at `submitted` was never claimed, though the batch had run and been charged.** Sibling to the resume fix above and hit on the same live run: a collect returned after saving the synthesis while the triage batch was still in flight, leaving `triage: {"status": "submitted"}` in the state file. The batch completed server-side seconds later and the result sat there unclaimed forever, because the post-pass list is built from `pending` entries only — so nothing polled it again, and the work order the user had paid for could not be retrieved. Collect now polls any pass left at `submitted`, alongside the ones it needs to submit, and does so regardless of the `include_synthesis` / `include_triage` flags: the spend already happened, and discarding a finished result is worse than saving one the caller had opted out of. It never re-submits such a pass, so nothing is charged twice. Verified by recovering the real stranded triage from the live scan.

- **Four MCP handlers trusted an argument their siblings already guard** — found by the first live Broad-Side scan of this repository, which flagged each one. An MCP server is handed whatever JSON a client sends, and in each case the failure was not a clean rejection. `codecarto_validate` and `codecarto_complete` called `args.phase?.trim()` with no type check, so a numeric `phase` threw a bare `TypeError` that surfaced as an opaque internal error rather than naming the bad argument. `codecarto_vision` never checked `raw_text`, which is interpolated straight into the returned prompt — omit it and the agent was asked to synthesize a vision brief from the literal word `undefined`. `codecarto_config` and the config load behind `codecarto_publish`'s `publish_confirm` gate accepted a relative `cwd` and resolved it against the *server process's* working directory, quietly answering from whatever workspace happened to sit there. All four now validate the way `validateCwd` and `handleSkill` already did.

- **The cost estimate budgeted one request's output per lens, however many requests the lens sent.** `estimateCost` summed input across every slice but computed output as a single `maxTokens * 0.75` for the whole lens — so a lens that sliced this repository into 13 modules budgeted one request's output and shipped thirteen. Since the estimate is what `max_cost` binds against, a run could outspend its cap several times over while the pre-flight breakdown looked reassuring. Output now scales with the slice count, a lens with nothing to scan budgets nothing, and the system prompt and JSON schema that ride on every request are counted when the caller passes repo info (they were free before). Measured against a live six-lens self-scan of this repository: OpenRouter holds the worst case at submission — every request's full `max_tokens` — which was `$0.6972` here, then refunds the unused output as each request settles, landing at `$0.3806`. The estimate was `$0.1174` before this and the pricing fix above (3.2x under the settled cost, 5.9x under the hold), `$0.2351` with the rate corrected, and `$0.5573` with both: now between the two, above what the run will settle at and below what it can hold. That is the right place for a number `max_cost` binds against. The hold is worth knowing about when budgeting a run: a balance dips by the ceiling before it returns to the real cost, so a scan that ends up costing 38 cents needs 70 cents available to start. This is now stated in `.codecarto/broadside/SKILL.md`.

- **A collect interrupted before its post-passes could never produce the synthesis or triage, however many times it was re-run.** The executive report and the P0–P3 work order are gated on having lens findings in hand, but a collect only holds the results *it* polled. When an earlier collect saved every lens and then died — the batch window is long and a poll runs for minutes, so this is an ordinary outcome rather than an edge case — the next collect found every lens already terminal, skipped them all, reached the post-pass gate with nothing, and marked the run `completed` with `synthesis: pending` and `triage: pending` frozen in the state file. The run kept its 43 per-lens files and permanently lacked the two artifacts it exists to produce. A resumed collect now reads the saved results back off disk (`loadSavedLensResults`), so "the state file tracks each so a resumed collect can finish whichever is still pending" is true rather than aspirational. Found by a live scan whose first collect was interrupted.

- **A lens whose batch never came back was omitted from the report rather than reported as missing.** When a poll budget expires with a batch still in flight, the poll returns a synthetic `{ status: "timeout" }` that carries no `error` object. The collect loop recorded an outcome only for a batch that completed *or* carried an error, so the timed-out lens was written to the state file correctly and then left out of `lensOutcomes` — which is what the caller renders. In the output, a lens that never returned was indistinguishable from one that was never requested, and the run's own summary listed five lenses where six had been paid for. Every non-completed outcome now reaches the report with its status.

- **A post-pass whose batch expired sat at `submitted` forever and was re-polled by every later collect.** The synthesis and triage passes were retired only when the batch carried an `error`, but `expired` and `cancelled` come back terminal and error-less. The pass stayed `submitted`, which is exactly the state the resume fix above treats as "still claimable" — so each future collect re-polled a batch that would never produce anything. Dead statuses now retire the pass. A `timeout` still does not: that batch is running server-side and has already been charged, so a later collect must be able to claim its result. The distinction is now a named constant (`BROADSIDE_DEAD_BATCH_STATUSES`) shared with the poll loop, rather than two hand-maintained lists.

- **An error during `submit` with `wait_seconds` reached the client as an opaque internal error.** `handleBroadside` normalizes failures from `runBroadsideSubmit` and from the `collect` action into an `McpError` naming the cause, but the `runBroadsideCollect` call on the submit-with-wait path had no such handler — the one call of the three that was missing it. Anything that went wrong while waiting was reported without its message.

- **An incremental scan that could not find a baseline silently became a full-price one.** Incremental scouting diffs against the commit an earlier run recorded, and falls back to scanning everything when there is nothing to diff against — no earlier run recorded a commit, or the working tree is dirty so there is no committed state to compare. The fallback is the right behavior; saying nothing about it was not. The caller asked for the cheap mode, was charged for the expensive one, and had nothing in the output to tell the two apart: a scan that reports six requests looks the same whether incremental narrowed it to six or never applied at all. Found live, scanning an unrelated Rust repository — the run submitted all 31 requests at full price because the earlier run predated the directory being a git checkout, and the summary mentioned none of it. Submit now states whether incremental applied, names the commit it diffed against when it did, and gives the specific reason when it did not. A run that never asked for incremental still says nothing.

- **`wait_seconds` bounded only the first of a collect's three polling phases.** A collect polls three times: the lens batches, then any truncated slices it re-submits, then the synthesis and triage passes. Only the first honored the caller's deadline — the other two each started a fresh 25-minute budget, so `wait_seconds: 60` could block for the better part of an hour, and the documented contract ("poll up to this many seconds before returning with partial state") held for one phase out of three. All three now share one deadline. A post-pass whose poll runs out is left at `submitted` rather than discarded, so the batch stays claimable by a later collect and nothing that was paid for is lost. Found while watching two live scans run well past the budget they were given.

- **The submit summary counted lenses considered, not batches sent.** A lens that gathers no files is reported as `skipped (0 request(s))`, but the header above that list counted every entry in the result, so it contradicted its own body. A live scan of a Rust command-line tool with no server surface announced "Broad-Side submitted 6 batch(es)" over a list showing four batches and two skips. The header now counts batches that were actually sent and says how many lenses produced none.

- **Three unreachable-but-inconsistent guards, tightened.** `handleStatus` read `phases[currentPhase]?.open_questions.length` — the optional chain covers the phase, not the field, while the reduce two lines below writes `phase.open_questions?.length`. `normalizeStatus` backfills the array, so the crash is not reachable through `getWorkspaceState`; this was verified empirically before being classed as hygiene rather than a defect. Separately, `walkFiles` derived relative paths with `slice(rootDir.length + 1)`, which cuts one character too many when the root carries a trailing separator; it now uses `relative()`. Third, `BroadsideCatalogResult.entry` was declared `CatalogEntry | null` while every path in `resolveCatalogEntry` returns an entry or throws — the nullable declaration was the only reason its single consumer needed an unchecked non-null assertion to read it. The type now says what the function does and the assertion is gone; `tsc` proves no path returns null. All the items above came from live Broad-Side scans, of this repository and of an unrelated Rust codebase, each verified against the source before any change.

- **The spend guardrail priced the default batch model at half its real cost.** `BROADSIDE_INPUT_PRICE_PER_M` / `BROADSIDE_OUTPUT_PRICE_PER_M` were `0.1875` / `0.9375`, but OpenRouter lists `google/gemini-3.7-flash:batch` at `0.375` / `1.875` — the `:batch` variant's price already carries the batch discount off the `0.75` / `3.75` sync rate, and the discount had been applied a second time by hand. Every estimate for the default model came out at half its true cost, so `max_cost` bound at roughly twice what the user asked for and the pre-flight breakdown understated the run: a real self-scan of this repository estimates `$0.2351`, not the `$0.1174` it used to report. Found by running a live scan and noticing the `models` action's catalog price disagreed with the estimate's. **The precedence was the deeper half of the bug:** `resolveCatalogEntry` returned the compile-time entry for the default model *before* consulting the cache or the live catalog, so a stale constant could never self-correct even though the catalog was already being fetched for every other model — and the test covering it was already named "falls back to built-in … without network", describing an intent the code had drifted from. The cache and the live catalog now win for the default model too, and the constants are the offline fallback they were documented to be. The example rates in `.codecarto/broadside/config.yaml` are corrected to match.


- **The YAML parser rejected any key named after a member of `Object.prototype`.** Duplicate detection asked `key in result`, and a bare object inherits from `Object.prototype` — so `"constructor" in result` was already true before a single line had been parsed. A document containing `constructor:`, `toString:`, `valueOf:` or `hasOwnProperty:` was refused as a duplicate key on first sight. Assignment had the matching hole: `result[key] = value` with the key `__proto__` invokes the prototype setter rather than creating an entry, so a hand-edited file could reshape every object in the process instead of parsing. Keys are now tracked in a set and written with `defineProperty`. Genuine duplicates, including duplicated prototype names, are still rejected.

- **A list item indented past its dash was rejected as bad indentation.** A sequence item's own mapping was parsed at a fixed two columns past the dash, so `-   id: architecture` with its siblings aligned under the `id` — an ordinary way to lay out a list — threw `Invalid YAML indentation`. The child mapping now continues at the column where the item's content actually starts. This matters because pipeline files, `status.yaml` and the Broad-Side config are all hand-editable, and the shipped two-space style was the only one that worked. Both parser defects were found by a live Broad-Side scan of this repository and confirmed against the parser before being fixed; `core/yaml.ts` now has direct test coverage, which it did not before.

## [0.19.0] — 2026-09-07

The closure-integrity round. Stages 1 and 2 of #122 made a run unable to ship a finding whose own evidence cell contradicted its action; Stage 3 closes the other half — a phase could still *close* a routed item while the question it came from stood open, which is the precise shape the original report described. Three additive fields do it: `derives_from` links a routed candidate to the question it answers and refuses a closure that outruns it, `open_question_closures` grows an evidence form that a `needs-runtime-test` question now requires, and completed phases' declared coverage gaps travel into the next phase's prompt so a blind spot upstream cannot license an observed fact downstream. Every field is optional and the schema version is unchanged, so existing handoffs parse exactly as before; the one rule that can bind an old-shape handoff is scoped to workspaces whose scaffold documents it.

### Added

- **Closure integrity in the handoff schema: a phase can no longer close a routed item while the question it came from is still open** (#122, #186). Stage 3 of the #122 investigation, held back from v0.17.1 because it changes the phase handoff — ABI for every existing workspace. Stages 1 and 2 read what a finding's own cells said; nothing verified that a *closure* was honest, and `applyHandoff` removed `carry_forward` and `open_questions` entries by id unconditionally. Three additive pieces close that. **`derives_from` on a carry-forward entry** records that the routed item is one candidate answer to a named `open_questions` id — filled in the handoff that registers the question, which is the one moment both entries are in front of the phase writing them. Completion now refuses a `carry_forward_closures` entry whose `derives_from` question is still unresolved and is not closed by the same handoff, naming both ids and saying what to do: close the question here with evidence, or leave the item routed and give the finding `verify at runtime`. That is exactly the shape #122 reported, where a routed `logit_bias` hypothesis shipped as `strong inference` / `fix before porting` while the question saying "source alone cannot determine which" stayed open. **`open_question_closures` accepts `{id, evidence}`** alongside the bare id it has always taken, and for a question whose `kind` is `needs-runtime-test` the evidence is required and must be non-empty — such a question closes on a spike report or an observation against the running system, not on another read of the same source. Presence is the gate; what the evidence *says* stays prose guidance. **Completed phases' declared coverage gaps now reach the next phase's prompt**: the new `core/coverage.ts` parses the `## Coverage and limits` ledger whose five bullet labels are fixed across every template, and the orchestrator-duties block lists each completed phase's `Skipped scope` and `Known blind spots`, with the duty that a finding inside one of them must either close it with cited new evidence or inherit its uncertainty. This is the duty the contradiction sweep was missing — that sweep compares against `owner_notes`, and a declared blind spot is not an owner note — and it is what would have caught #122's second failure, where a phase asserted an `observed fact` about a component an upstream phase had recorded as not fully decoded. It is non-gating prompt text, capped and rendered like the re-triage list, and both surfaces get it from `buildPhasePrompt`. Both refusals are deterministic reads of ids the model wrote itself and sit before the status lock, so a refusal mutates nothing and neither can wedge `--auto` on a heuristic. **Back-compatibility:** every new field is optional and the schema version is unchanged, so an existing workspace's handoffs parse exactly as before — a bare-string closure still normalizes to `{id}`, a `carry_forward` entry without `derives_from` closes without comment, and a phase output without a `## Coverage and limits` section contributes nothing to the prompt. The runtime-evidence requirement is the one rule that can bind a handoff using none of the new fields, since a bare-string closure was the only shape before this release — a gate bound to the object form alone would be one an unchanged handoff could always sidestep. So it is version-gated the way Stage 2's pairing check is: refused on a workspace whose `scaffold-version.yaml` is 0.19.0 or newer, and reported as a non-gating `NOTE:` on an older or unversioned scaffold, whose own templates never stated the rule. An in-flight run cannot be stopped by a requirement it was never told, and refreshing the scaffold opts a workspace in.
