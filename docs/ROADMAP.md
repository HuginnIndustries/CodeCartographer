...
|---|---|---|---|
| Foundation | Existing specs/plans expose useful slice/scenario links and verification seams | Real phase output plus invariant/compatibility tests, not only template edits | E09 delivered (slice tables, ids, lifter, compatibility test against shipped self-audit specs); E10 delivered (Agent Addressability table with evidence levels in architecture and porting, Verification route per slice lifted in E01 `check_kind` vocabulary, `none` refused as a gap; hand-filled dogfood against this repo); E11 partially verified: the loop and negative controls N01–N08 plus second-change continuity run through real records in `tests/engineering-e2e.test.mjs` (docs/engineering/pilot-e11-2026-09-22.md); the live-host F01 feature pilot and an actual human acceptance remain open |
...
| First local loop | A host can plan, implement, verify, review, and present a bounded change without a library | Real fix and feature; failed attempt/resume, stale-proof refusal, actual acceptance, second-change history | E02–E08 delivered; E11 partially verified (record-level loop, failed-attempt resume, stale-proof refusal, second-change history in `tests/engineering-e2e.test.mjs`); live-host feature pilot and actual acceptance still open |
