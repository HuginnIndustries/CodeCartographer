...
- [`2026-09-15-self-audit-closeout.md`](2026-09-15-self-audit-closeout.md) — the session that split `core/broadside.ts`, ran the full deep-audit pipeline on the repository unattended through Pi (27 findings → issues #355–#372), and made CodeCartographer the subject of its own README and website. Its outcome section records the next day: v0.26.0 shipped the batch (sixteen fixes, each proven by a test that fails on the previous code; #363 closed as designed), and the Windows CI job it asked for found #393–#395.
- [`2026-09-14-verification-loop-closeout.md`](2026-09-14-verification-loop-closeout.md) — the session that turned `verify` on the repository itself and shipped 0.24.1 and 0.25.0: two verified findings fixed, the defect-lens rubric measured (two runs per arm) before it was adopted, two defects the measurement runs surfaced, why rename-then-rm does not fix a stale-lock race, and the work order built from the verdicts.
- [`2026-09-13-live-verification-closeout.md`](2026-09-13-live-verification-closeout.md) — the session that closed the issue board and ran the framework against real providers: five releases (0.22.0–0.23.0), seven defects mocks could not show, the decisions they forced, and the verification recipes that came out of it.
