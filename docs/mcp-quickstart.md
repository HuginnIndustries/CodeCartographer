...
codecarto_broadside {cwd: "/abs/path/to/repo", action: "collect", regenerate_post_passes: true}   # work order ranked on the verdicts
```
...
- **`codecarto_next` says all phases are complete** — they are; `codecarto_status` shows `Pipeline state: complete`. If it instead errors with **"Pipeline is stuck: …"**, a remaining phase depends on a phase the pipeline file does not declare (or on a cycle); nothing can run until that `depends_on` is fixed or you switch pipelines. `codecarto_status` shows `Pipeline state: stuck` and names the phase.
