...
| `classifyAcceptance(approval, context + current_storage [+ integrations])` | `verified` / `cooperative` / `invalid` for the reader, now: re-derives the integration from the registry, requires the boundary host-enforced now with an `enforced_since` that precedes `decided_at` |
...
| Approval decided before the boundary's `enforced_since`, or read under a boundary of `none`, however its `storage` field is spelled | `classifyAcceptance` → `cooperative` |
...
`classifyAcceptance(approval, context)` is how a downstream reader (E06's gate, E07's status, E08's resume) tells them apart. It is `verified` only when the receipt binds; the receipt's `(host, client.name, channel)` is in `VERIFIED_ACCEPTANCE_INTEGRATIONS` **now**; the policy was `verified`; the record says the boundary was host-enforced at mint; **and** the reader's `CurrentStorage` is `{ boundary: "host-enforced", enforced_since }` with `enforced_since` earlier than the approval's `decided_at`. Any other bound receipt is `cooperative`; a receipt that does not bind is `invalid`. A real elicitation answer stored in a namespace agent tools can write is therefore `cooperative` — the channel does not protect the file, and neither `host-native` nor `mcp-elicitation` implies that it does.

Why the epoch: the record's own `storage.boundary` and `receipt.verified_integration` are bytes in the file. Under `boundary: none` an agent can write `host-enforced` and `true` into them; if the reader only checked its current boundary, enabling the boundary later would retroactively make that forgery `verified`. So a boundary counts only from the instant it has been continuously enforced, recorded in host configuration outside the namespace (D3), and every approval decided before that instant is cooperative however it is labelled. The test suite pins the forged-record case in both directions.
...
| D3 | Which **storage-boundary mechanism** will the pilot host provide (a client permission rule denying agent writes under `.codecarto/engineering/`; a namespace outside agent-writable roots; a Pi build-mode guard — the last is a separate guard-policy change and not assumed here)? Where do the boundary declaration and its `enforced_since` live so that agent tools cannot write them, and must the boundary predate the namespace's first write? | `storage_boundary` is adapter-declared; `verified` requires `host-enforced` now with an epoch before the decision |
| D4 | Which **observed-evidence path** will the pilot host provide (a post-tool-use hook delivering tool results to a host-side ingestion entry, defined by E05)? Must that entry be demonstrably unreachable by the model's tools, and must the hook's configuration live outside agent-writable roots, for it to count as `host-tool-result`? Without one, every proof is `claimed` and nothing is accepted under `verified`. | `proofDischarges` refuses claims under `verified`; the record requires `tool_call_id` and a `host-observed` collector |
