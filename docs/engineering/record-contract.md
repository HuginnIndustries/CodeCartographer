...
Two evasions are refused at the schema, because both let the sweep be claimed without being done: an empty `instances` list, and an `out-of-scope` instance with no `note`. The field itself stays **optional** — its absence is an R11 finding for the evaluator, not a malformed record — so records written before R11 existed remain valid. Widening a fix without bound is its own defect; what R11 forbids is leaving the question unasked, since an implementer who never named the class cannot claim the remaining instances are out of scope.
