...
| `elicitationDecision(response)` / `acceptanceTtlWithin(request, integration)` | the outcome of an elicitation round trip, from `content.decision` only; whether the request's TTL fits inside the client's observed request timeout |
...
- **Timeout is a third outcome.** `ElicitationOutcome` has `timed-out` distinct from `rejected`, `declined`, and `cancelled`; `AcceptanceRequestOutcome.outcome` carries `declined` and `timed-out`; neither mints a record. The adapter's TTL must be shorter than the client's request timeout (`VerifiedAcceptanceIntegration.client_request_timeout_ms`, observed ~150 s on this pair; `acceptanceTtlWithin`), and the adapter should pass that TTL as the `elicitInput` request timeout so the server-side error is deterministic. A person at an approval prompt routinely takes minutes; E06's TTL for this channel is bounded by the client, not by the 24 h contract maximum.
