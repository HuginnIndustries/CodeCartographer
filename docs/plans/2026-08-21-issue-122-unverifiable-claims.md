...
**Status:** Stages 1 and 2 shipped in v0.17.1; Stage 3 is deferred and tracked in [#186](https://github.com/HuginnIndustries/CodeCartographer/issues/186).

> **What shipped (v0.17.1).** Stage 1's vocabulary — the `external-behavior claim` evidence level and the
> `verify at runtime` action — across every SKILL, pass file, template, pipeline criterion and doc, plus the
> pairing rules, the `## Open Questions` tables in the three defect templates, the cite-or-hedge rule for
> quantities, and the re-triage counterpart duty in GUIDE.md and the orchestration guide. Stage 2's mechanical
> checks live in `core/findings.ts`: the pairing violation fails validation on a current scaffold and warns on an
> older one (version-gated on `workflow/scaffold-version.yaml`), and non-gating notes cover a self-contradicting
> finding, an empty Open Questions table, and a closure the primary output never mentions.
>
> **What is deferred (#186).** Stage 3 — `derives_from` on carry-forward entries, evidence on
> `open_question_closures`, and carrying the coverage-gap ledger into the next phase's prompt. All three change
> the phase-handoff schema, which is ABI for every existing workspace, so they were held for their own
> discussion rather than ridden along in a fix release. The options and constraints below are what that work
> starts from.
