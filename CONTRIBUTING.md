...
| Surface | Run | Record |
|---|---|---|
| Pi | `/codecarto-init architecture-only` then `/codecarto-next --auto` in a scratch repo, on a real model (cheapest known: `--model openrouter/google/gemini-3.7-flash` with `OPENROUTER_API_KEY`, ~$0.25 for a phase on a ten-file repo; a provider registered by one of your own Pi extensions works too, but `-ne` disables extension discovery, so pass that extension with `-e` alongside the packed one) | the notifications through "Auto pipeline complete", `current_phase: complete`, the artifact's `**Overall:**` line — and when the loop stops short, the reason: it is in the auto-summary message and the widget, and under `pi -p` only there, so reproduce a completion refusal with `completeValidatedPhase` from the tarball's `dist/core/index.js` |
...
The batch list is how a duplicate post-pass or retry shows up — the collect report cannot see a batch another process submitted.

**A lens prompt change is measured, not eyeballed.** The 0.24.1 defect-lens rubric (#341) was decided on two runs per arm, ~$0.40 and fifteen minutes: pin a target checkout (`git worktree add --detach <scratch>/target <sha>`), run one driver per arm that imports each branch's `core/broadside.ts` barrel (a scratch worktree of the branch; `core/` has no npm dependencies), submit both in parallel — a defect sweep of this repository finishes in four to eight minutes — and judge every medium-severity finding by hand against the source *before* reading `verify`'s verdicts. Report findings per severity, true findings per run, and cost; either prompt misses a real defect on some runs, so a difference smaller than the run-to-run spread is not a result. Drive the server the way a host does (an MCP client over stdio against the tarball's `bin.mjs`) and give the client a request timeout longer than `wait_seconds`; the SDK's default is 60 s, and a client that gives up mid-wait is the #322 scenario, which is worth reproducing on purpose once: the server must exit within seconds and the next collect must submit exactly one synthesis and one triage. Batch jobs take five to twenty minutes on a tiny repository, so run the collect in the background and check back.
