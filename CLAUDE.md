...
- `core/broadside.ts` — Broad-Side batch reconnaissance; a barrel over `core/broadside/`: `constants`, `types`, `schemas`, `lenses` (registry and prompts), `repo` (listing, language detection, lens scoping and fallback, slicing), `requests`, `state` (the `.codecarto/broadside/` state file, claims, config), `models` (catalog, pricing, endpoint memory), `client` (OpenRouter Batch API and provider-error explanations), `submit`, `results` (stored lens results), `collect` (polling, retry, synthesis and triage post-passes), `verify` (the read-only-tools verification pass), `render` (the text both surfaces print). The graph is acyclic at runtime and points one way through the layers the barrel lists — `tests/module-graph.test.mjs` pins both, and that no `core/` module imports `./index.ts`; `core/index.ts` re-exports the barrel. Executable surfaces only (Pi via `/codecarto-broadside`, MCP via `codecarto_broadside`); the template carries just the reading guide.
