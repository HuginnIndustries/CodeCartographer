...
export const ACCEPTANCE_REQUIRED_RECORDS = [
	...
	{ kind: "approval", requirement: "`decision: accepted`; receipt passes `evaluateApprovalReceipt`; `classifyAcceptance` is `verified` — trusted channel on a currently registered integration, `verified` policy, storage boundary host-enforced at mint, and the reader's out-of-namespace `CurrentStorage` host-enforced with `protection: continuous-since-initialization` (no record-internal timestamp substitutes) — or, only under an operator-set `cooperative` policy, `cooperative`" },
] as const;
